    ...
    return (
        <Tooltip title={`There were ${ratings_count} ratings `} placement="top-start" arrow>
            <svg className="CircleRating">
                ...
                <path className={`CircleRating-percent--${addColorRating(percentageRating)}`} strokeDasharray={`${percentageRating}, 100`} d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"/>
                <text className={`CircleRating-text--${addColorRating(percentageRating)}`} x="18" y="14" textAnchor="middle" dy="7">{`${percentageRating}%`}</text>
            </svg>
        </Tooltip>
    )
}
